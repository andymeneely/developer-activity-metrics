...
public class LaplaceHashEmissionPDFTest {
	...
	public void noSetProb() throws Exception {
		try {
			pdf.setProbability(ChurnSignal.BIG_CHANGE, 1);
	...
	public void incrementBy() throws Exception {
		pdf.incrementOccurrence(ChurnSignal.BIG_CHANGE, 2);
		...
		assertEquals("everything else", 0.1666, pdf.getProbability(ChurnSignal.BIG_NEW_CODE), 0.01);
		assertEquals("everything else", 0.1666, pdf.getProbability(ChurnSignal.SMALL_CHANGE), 0.01);
		assertEquals("everything else", 0.1666, pdf.getProbability(ChurnSignal.SMALL_NEW_CODE), 0.01);
		assertEquals("everything else", 0.5, pdf.getProbability(ChurnSignal.BIG_CHANGE), 0.01);
	}

	@Test
	public void addsUpToOne() throws Exception {
		pdf.incrementOccurrence(ChurnSignal.BIG_CHANGE, 2);
		pdf.incrementOccurrence(ChurnSignal.BIG_CHANGE, 3);
		pdf.incrementOccurrence(ChurnSignal.SMALL_CHANGE, 2);
		pdf.incrementOccurrence(ChurnSignal.SMALL_NEW_CODE, 26);
		double total = 0;
		for (ChurnSignal signal : ChurnSignal.values()) {
